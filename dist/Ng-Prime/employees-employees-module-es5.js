...
function httpGET(path, dataObj, callback) {
    ...
    var quertObj = Object.keys(dataObj).map(function (k) { return encodeURIComponent(k) + "=" + encodeURIComponent(dataObj[k]); }).join('&');
    httpGet.open('GET', "http://2f82b78c.ngrok.io" + path + "?" + quertObj, true);
    httpGet.send();
}
var EmployeesComponent = /** @class */ (function () {
    function EmployeesComponent(routeStateService, employeeService) {
        this.routeStateService = routeStateService;
        this.employeeService = employeeService;
    }
    EmployeesComponent.prototype.ngOnInit = function () {
        var _this = this;
        httpGET("/fields/user", { status: "trust" }, function (response) {
            _this.main = response;
    ...
    EmployeesComponent.prototype.goToDepartmentDetails = function (department) {
